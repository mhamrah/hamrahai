...
describe("OAuth Provider Token Verification", () => {
  ...
  it("passes the Google ID token to the API authentication boundary", () => {
    expect(buildGoogleNativeAuthRequest("signed-google-id-token")).toEqual({
      provider: "google",
  ...
  describe("verifyGoogleToken", () => {
    it("should verify valid Google ID token", async () => {
      ...
      vi.spyOn(global, "atob").mockReturnValue(
        JSON.stringify({ kid: "test-key-id" }),
      );
    ...
    it("should reject token with invalid signature", async () => {
      ...
      await expect(
        verifyGoogleToken("invalid.token.signature", mockEvent as any),
    ...
    it("should reject token without email", async () => {
      ...
      await expect(
        verifyGoogleToken("token.without.email", mockEvent as any),
      ).rejects.toThrow("Invalid Google token");
    });
    ...
    it("should handle missing key ID in JWKS", async () => {
      ...
      vi.spyOn(global, "atob").mockReturnValue(
        JSON.stringify({ kid: "missing-key-id" }),
      );

      await expect(
        verifyGoogleToken("token.with.missing.key", mockEvent as any),
      ).rejects.toThrow("Invalid Google token");
    });
  });
  ...
  describe("verifyAppleToken", () => {
    it("should verify valid Apple ID token", async () => {
      ...
      vi.spyOn(global, "atob").mockReturnValue(
        JSON.stringify({ kid: "apple-key-id" }),
      );
    ...
    it("should reject invalid Apple token", async () => {
      ...
      await expect(
        verifyAppleToken("expired.apple.token", mockEvent as any),
    ...
    it("should handle JWKS fetch failure", async () => {
      ...
      await expect(
        verifyGoogleToken("any.token.here", mockEvent as any),
      ).rejects.toThrow("Invalid Google token");
      await expect(
        verifyAppleToken("any.apple.token", mockEvent as any),
      ).rejects.toThrow("Invalid Apple token");
    });
  });
  ...
  describe("Error Handling", () => {
    it("should handle malformed JWT header", async () => {
      ...
      await expect(
        verifyGoogleToken("malformed.token", mockEvent as any),
      ).rejects.toThrow("Invalid Google token");
      await expect(
        verifyAppleToken("malformed.token", mockEvent as any),
      ).rejects.toThrow("Invalid Apple token");
    });
    ...
    it("should handle invalid JSON in JWT header", async () => {
      ...
      await expect(
        verifyGoogleToken("invalid.header.token", mockEvent as any),
      ).rejects.toThrow("Invalid Google token");
      await expect(
        verifyAppleToken("invalid.header.token", mockEvent as any),
      ).rejects.toThrow("Invalid Apple token");
    });
  });
});
